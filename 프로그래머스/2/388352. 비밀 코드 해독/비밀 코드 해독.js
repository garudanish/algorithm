...
function solution(n, q, ans) {
    ...
    const possibleCombinations = generateCombinations(numbers, 5)
        .map((combinations) => combinations.reduce((map, number) => map.set(number, 1), new Map()));
    
    let answer = possibleCombinations;
    
    for (let i = 0; i < q.length; i += 1) {
        answer = answer.filter((countMap) => q[i].reduce((acc, cur) => acc + (countMap.get(cur) || 0), 0) === ans[i]);
    }
